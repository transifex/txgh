...
describe GitlabRepo do
  ...
  describe '#gitlab_config_branch' do
    context 'with all branches indicated' do
      let(:branch) { 'all' }

      it "doesn't modify the passed branch, i.e. returns 'all'" do
        expect(repo.gitlab_config_branch).to eq('all')
      end
    end
    ...
    context 'with a nil branch' do
      ...
      it 'chooses master by default' do
        expect(repo.gitlab_config_branch).to eq('heads/master')
      end
    end
    ...
    context 'with a configured branch' do
      ...
      it 'correctly prefixes the branch' do
        expect(repo.gitlab_config_branch).to eq('heads/foobar')
      end
    end
  end

  describe '#gitlab_config_tag' do
    context 'with all tags indicated' do
      let(:tag) { 'all' }

      it "doesn't modify the passed tag, i.e. returns 'all'" do
        expect(repo.gitlab_config_tag).to eq('all')
      end
    end
    ...
    context 'with a nil tag' do
      ...
      it 'returns nil' do
        expect(repo.gitlab_config_tag).to be_nil
      end
    end
    ...
    context 'with a configured tag' do
      ...
      it 'leaves the prefix intact' do
        expect(repo.gitlab_config_tag).to eq('tags/foobar')
      end
    end
  end
